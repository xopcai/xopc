...
function normalizePrefs(raw: unknown): DesktopPetPrefs {
  ...
  const feedbackLevel =
    obj.feedbackLevel === "quiet" ||
    obj.feedbackLevel === "normal" ||
    obj.feedbackLevel === "chatty"
      ? obj.feedbackLevel
...
export async function readDesktopPetState(
  ...
  const [prefs, petList] = await Promise.all([
    readDesktopPetPrefs(),
    listDesktopPets(),
  ]);
