...
Explicit preview limits: one paired Gateway; file-manager upload 8 MiB, download 16 MiB, text preview 1 MiB; chat attachments up to 10 files, 10 MiB each and 20 MiB total, selected through the system document picker; voice upload 4 MiB; no background recording. Chat now has structured execution/review/deliverable cards, image previews, Markdown tables and safe links; media reads are bounded to 16 MiB and thumbnails to a 1024px longest edge. Historical audio aliases and sent-attachment previews are supported; offline audio progress/pause/cache cleanup and text preview pass emulator fixtures. Video, remote media transfers, physical audio and product sharing require the remaining acceptance described in [Chat parity](docs/chat-parity-checklist.md). Markdown remains a native subset without executable HTML or a rich editor. No Live View, system service cards, live voice conversation or distributed-device features. Large-data pagination/layout/performance and accessibility are release gates, not implied by a successful build.
