...
Baseline: `apps/mobile-expo/src/features/chat/ChatScreen.tsx`, the components it composes, and `CapsuleTabBar.tsx` (2026-09-17 working tree). Harmony must not replace the main conversation when opening a history detail. This checklist is an acceptance inventory, not a completion claim.

| Area | Acceptance |
| --- | --- |
| Drawer | Left overlay; edge swipe; scrim/back/close dismissal; keyboard dismissal; profile and four shortcuts; automation shortcut; dated, paginated history; selected indicator; search; new conversation; root draft survives a history detail |
...
| Header | Model title and picker; agent picker creates an isolated conversation; conversation actions sheet; conversation-scoped files; back on details |
