...
| 文档预览/分享 | `AttachmentRenderer`、`FilePreviewModal`、HTML 预览 | 本轮已补普通附件紧凑列表（3 条折叠）、统一全屏预览头、图片/Markdown/文本/HTML/音视频/二进制分流、提取文本降级、分享下载；HTML 使用 CSP、禁 JS/存储/文件/外部跳转。PDF/Office 与 Android 一样不伪装为内嵌渲染；真机视觉与系统分享仍待验收 | P1 验收 |
