...
const originalHistory = nodes.find(item => item.id?.startsWith('drawer-session-') && item.selected === 'true');
assert(originalHistory, 'Current main conversation must be in the visible history fixture');
const history = nodes.find(item => item.id?.startsWith('drawer-session-') && item.id !== originalHistory.id); assert(history, 'Another history fixture is required');
click(history); await delay(1000); nodes = await tree('detail');
node(nodes, 'main-tab-dock'); node(nodes, 'chat-open-drawer');
await screen('detail'); click(node(nodes, 'chat-open-drawer')); nodes = await tree('restore-history');
assert.equal(node(nodes, history.id).selected, 'true'); click(node(nodes, originalHistory.id));
nodes = await until('restored-root', items => items.some(item => item.id === 'chat-composer' && item.text === '/'));
assert.equal(node(nodes, 'chat-composer').text, '/'); node(nodes, 'main-tab-dock'); passed.push('history switches the main Chat and restores its per-conversation draft');
...
node(nodes, 'main-tab-dock'); node(nodes, 'chat-open-drawer');
assert(!nodes.some(item => item.id === 'sessions-list'));
click(node(nodes, 'chat-open-drawer')); nodes = await tree('sessions-return-drawer');
click(node(nodes, originalHistory.id));
nodes = await until('sessions-restored-root', items => items.some(item => item.id === 'chat-composer' && item.text === '/'));
assert.equal(node(nodes, 'chat-composer').text, '/'); node(nodes, 'main-tab-dock');
passed.push('session manager search, long-press menu, rename cancel, explicit selection, batch-delete cancel and main-Chat selection preserve per-conversation drafts');
