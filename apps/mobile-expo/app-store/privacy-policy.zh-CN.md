...
**尚不可发布。** 请先填写法定姓名和联系邮箱，并完成发布前复核。本文依据 2026-09-04 检查的代码及已确认产品模式起草。
...
本政策说明个人开发者[法定姓名]发行的 xopc 移动端如何处理数据。App 免费提供，不含购买或订阅，仅连接由用户自行运行和管理的网关。发布者不向 App 用户提供托管网关、App 账号或强制使用的 AI 云服务。用户选择的网关、模型及其他服务有各自的运营者、隐私政策和数据处理责任。
...
App 在设备上保存偏好设置、工作区缓存、待提交内容、授权选择及最多 200 条使用／性能事件。经检查的发行包不包含远程分析、广告或第三方崩溃上报 SDK；使用测试版期间，TestFlight 本身可能向开发者提供 Apple 收集的崩溃和测试诊断信息。
...
你可以在相应工作区页面删除内容，在网关撤销设备，并联系网关运营者申请删除备份或下游服务中的数据。卸载 App 不会删除远程内容。对于[法定名称]控制的数据，请通过[邮箱]私下提出请求，并提供足以定位相关交互的信息；处理请求前，发布者可能合理核验你对相关账号、设备或邮箱的控制权。
