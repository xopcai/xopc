    ...
    <View style={[styles.screen, { backgroundColor: colors.surface.base }]}>
      ...
      <ScrollView contentContainerStyle={[styles.list, { paddingBottom: insets.bottom + spacing.xl }]}>
        <>
          ...
          <Pressable disabled={!title.trim() || create.isPending} onPress={() => create.mutate()} style={({ pressed }) => [styles.createButton, { backgroundColor: colors.accent.primary, opacity: pressed || create.isPending || !title.trim() ? 0.55 : 1 }]}><Text style={styles.createText}>{labels.confirmAndCreate}</Text></Pressable>
