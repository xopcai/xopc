  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.surface.base }]}>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        ...
        {view.data.desiredOutcomes.length ? <><Text style={[styles.sectionTitle, { color: colors.text.primary }]}>{labels.desiredOutcomes}</Text>{view.data.desiredOutcomes.map((outcome) => <View key={outcome.id} style={[styles.card, { backgroundColor: colors.surface.panel, borderColor: colors.border.default }]}><Text style={[styles.cardTitle, { color: colors.text.primary }]}>{outcome.title}</Text><Text style={{ color: colors.accent.primary }}>{outcome.status}</Text>{outcome.nextAction ? <Text style={{ color: colors.text.secondary }}>{outcome.nextAction}</Text> : null}</View>)}</> : null}
        <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>{labels.projectWork}</Text>
        {view.data.currentActions.map((item) => <Pressable key={item.id} onPress={() => router.push(`/work/${item.id}`)} style={[styles.card, { backgroundColor: colors.surface.panel, borderColor: colors.border.default }]}><Text style={[styles.cardTitle, { color: colors.text.primary }]}>{item.title}</Text><Text style={{ color: colors.accent.primary }}>{labels.status[item.phase]}</Text>{item.nextAction ? <Text style={{ color: colors.text.secondary }}>{item.nextAction.text}</Text> : null}</Pressable>)}
...
export function CreateWorkItemScreen() {
  ...
  const start = useMutation({
    mutationFn: () => startOutcome({ requestId, objective: title.trim(), projectId: projectId || undefined }),
    onSuccess: async (work) => {
      ...
      if (projectId) void queryClient.invalidateQueries({ queryKey: queryKeys.projectWorkItems(projectId) });
      void queryClient.invalidateQueries({ queryKey: queryKeys.home });
      if (noteId) void queryClient.invalidateQueries({ queryKey: queryKeys.notesAll });
      openChat(router, work.sessionKey, { replace: true });
    },
  });
  return (
    <View style={[styles.screen, { backgroundColor: colors.surface.base }]}>
      ...
      <ScrollView contentContainerStyle={[styles.list, { paddingBottom: insets.bottom + spacing.xl }]}>
        <>
          ...
          <TextInput multiline value={title} onChangeText={(value) => { setTitle(value); setRequestId(crypto.randomUUID()); }} placeholder={labels.outcomePlaceholder} placeholderTextColor={colors.text.tertiary} style={[styles.input, styles.intentInput, { color: colors.text.primary, borderColor: colors.border.default }]} />
          <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>{labels.projectLabel}</Text>
          <Pressable onPress={() => { setProjectId(''); setRequestId(crypto.randomUUID()); }} style={[styles.projectChoice, { borderColor: !projectId ? colors.accent.primary : colors.border.default, backgroundColor: colors.surface.panel }]}><Text style={{ color: colors.text.primary }}>{labels.letXopcChoose}</Text>{!projectId ? <Text style={{ color: colors.accent.primary }}>{labels.selected}</Text> : null}</Pressable>
          {(projects.data ?? []).map((project) => <Pressable key={project.id} onPress={() => { setProjectId(project.id); setRequestId(crypto.randomUUID()); }} style={[styles.projectChoice, { borderColor: project.id === projectId ? colors.accent.primary : colors.border.default, backgroundColor: colors.surface.panel }]}><Text style={{ color: colors.text.primary }}>{project.name}</Text>{project.id === projectId ? <Text style={{ color: colors.accent.primary }}>{labels.selected}</Text> : null}</Pressable>)}
          <Pressable disabled={!title.trim() || start.isPending} onPress={() => start.mutate()} style={({ pressed }) => [styles.createButton, { backgroundColor: colors.accent.primary, opacity: pressed || start.isPending || !title.trim() ? 0.55 : 1 }]}><Text style={styles.createText}>{labels.confirmAndCreate}</Text></Pressable>
          {start.error ? <Text style={{ color: colors.text.secondary }}>{start.error.message}</Text> : null}
        </>
      </ScrollView>
    </View>
  );
