...
import { executeMobileEndpointTool } from '../tools';
...
describe('mobile endpoint file tools', () => {
  ...
  it('uploads the single file explicitly returned by the system picker', async () => {
    ...
    await expect(executeMobileEndpointTool('mobile.file.pick', {}, context)).resolves.toEqual({
  ...
  it('does not upload when the system picker is cancelled', async () => {
    ...
    await expect(executeMobileEndpointTool('mobile.file.pick', {}, context)).rejects.toMatchObject({
      name: 'AbortError',
    });
  ...
  it('shares a bounded temporary text file and removes it afterwards', async () => {
    await expect(executeMobileEndpointTool(
      'mobile.file.share',
