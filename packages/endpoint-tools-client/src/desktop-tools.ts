...
export interface DesktopToolApis {
  ...
  shell?: { openExternalUrl(url: string): Promise<{ ok: true } | { ok: false; error: string }> };
  system?: { showEndpointNotification(input: { title: string; body: string }): Promise<{ ok: true } | { ok: false; error: string }> };
}
