...
describe('current automation contract', () => {
  it('preserves current conversation and notification policy fields', () => {
    const parsed = AutomationSchema.parse({ id: 'a', name: 'Review', enabled: true, trigger: { kind: 'manual' },
      ...
      conversationMode: 'continuous', notificationPolicy: 'none', completionWebhookUrl: 'https://example.com/hook' });
    expect(parsed).toMatchObject({ conversationMode: 'continuous', notificationPolicy: 'none', completionWebhookUrl: 'https://example.com/hook' });
  });
  it('accepts completion hooks and rejects obsolete run phases and events', () => {
    expect(AutomationRunSchema.shape.currentPhase.parse('completion_hook')).toBe('completion_hook');
    expect(AutomationRunSchema.shape.currentPhase.safeParse('after_run').success).toBe(false);
    expect(AutomationRunEventSchema.shape.type.parse('completion_hook.completed')).toBe('completion_hook.completed');
