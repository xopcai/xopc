...
你可以先把它当作一个使用自有密钥的本地助手；也可以把需要持续推进的结果保存为可验证的 Task，用 Project 共享上下文，用笔记收集输入，用工作流拆解复杂任务，再用定时或 webhook 自动化触发后续工作。所有入口连接的是同一套运行时，而不是几个互相失忆的机器人。
...
| **持续状态** | 持久会话、Task、Project、笔记、工作区与 Agent 记忆策略 | 工作可以恢复，不必每次从零解释 |
...
典型用法不是固定漏斗，而是按需组合：在终端处理代码，在手机上语音速记到 Notes，在项目页查看 Task、阻塞与失败的工作流，在每天的自动化中生成摘要，最后从任意入口继续同一个会话。
...
- 🧠 **有状态，不只是一轮上下文** — 会话、Task、Project、笔记、工作区和运行记录都有明确归属，长期工作可以恢复。
...
xopc 不是又一个聊天 UI。它更像一个本地优先的系统层：让同一个助手持续维护 Task 与上下文、运行自动化，并在多个入口中持续可用。

| 产品 | 更擅长 | 为什么还需要 xopc |
| --- | --- | --- |
| **Codex** | 终端、IDE 和云端的软件开发任务 | xopc 不只服务一个代码仓库：可持续推进的 Task 与 Project、BYOK/本地模型、定时循环、Gateway API、桌面/网页/手机和 IM 都共享同一套本地状态。 |
...
**它和普通聊天 UI 有什么区别？** — 聊天只是入口。xopc 还管理 Agent、持久会话、Task、Project、笔记、工作区、工作流、自动化和多端连接。
