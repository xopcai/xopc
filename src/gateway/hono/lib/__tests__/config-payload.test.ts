...
describe('buildSafeBrowserConfigForWeb', () => {
  it('keeps the v2 driver and policy blocks for config reloads', () => {
    const safe = buildSafeBrowserConfigForWeb({
      enabled: true,
      driver: { kind: 'playwright', headless: true },
      observation: { maxNodes: 180 },
      limits: { actionTimeoutMs: 30_000 },
      security: { privateNetworks: 'deny' },
    });
  ...
  it('masks remote driver credentials', () => {
    const safe = buildSafeBrowserConfigForWeb({
      enabled: true,
      driver: { kind: 'remote', provider: 'browserbase', apiKey: 'secret-key', projectId: 'project-1' },
    });

    expect(safe.driver).toMatchObject({ kind: 'remote', provider: 'browserbase', projectId: 'project-1' });
    expect((safe.driver as Record<string, unknown>).apiKey).not.toBe('secret-key');
  });
});
