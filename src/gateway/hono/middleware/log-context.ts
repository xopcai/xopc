 ...
 * all pino logs in the request inherit requestId / transcriptId without a child logger.
 */
export function logContextMiddleware() {
  return createMiddleware(async (c, next) => {
    ...
    const sessionHeader = c.req.header('x-transcript-id')?.trim();
    ...
    return runWithLogContext(
      {
        requestId,
        ...(sessionHeader ? { transcriptId: sessionHeader } : {}),
      },
      async () => {
