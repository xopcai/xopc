...
export async function buildWorkItemAgentContext(
  ...
  const text = [
    ...
    contextLine('Phase', item.phase),
    contextLine('Priority', item.priority),
    contextLine('Completion policy', item.completionPolicy),
    contextLine('Resolution', item.resolution),
    contextLine('Owner agent', item.ownerAgentId),
    contextLine('Description', item.description),
    contextLine('Next action', item.nextAction ? `${item.nextAction.text} [${item.nextAction.actor}]` : undefined),
    item.waits.some((wait) => !wait.resolvedAt)
      ? `Open waits:\n${item.waits.filter((wait) => !wait.resolvedAt).map((wait) => `- ${wait.kind}: ${wait.reason}`).join('\n')}`
      : null,
