...
export class CliToolProvider implements ExternalToolProvider {
  ...
  async execute(toolRef: string, args: Record<string, unknown>, approvalId: string | undefined, context: ExternalToolExecutionContext) {
    ...
    const argumentsHash = () => connectorArgumentsHash({ args: input, exportResult, instanceId: item.instance.instanceId, accountId: connection.accountId, connectionId: connection.id,
      actionRevision: action.revision, binaryVersion: item.adapter.binaryVersion, adapterVersion: item.adapter.version, principalId: item.principal.principalId, agentId: item.agentId, conversationId, objective: conversationId ? connectorObjectiveScope(conversationId) : undefined });
    ...
    const beforeExecute = () => {
      runSqliteWriteTransaction(db => {
        ...
        if (!freshPolicy || freshConnection?.status !== 'active' || getConnectorAccount(connection.accountId!)?.currentConnectionId !== connection.id
          ...
          || evaluateConnectorExecutionPolicy({ installation: freshPolicy, action: { scope: action.scope, curated: true }, accountId: connection.accountId, agentId: item.agentId, confirmed: Boolean(approvalId) }).decision !== 'allowed') throw new Error('Connector permission changed.');
        if (approvalId) {
