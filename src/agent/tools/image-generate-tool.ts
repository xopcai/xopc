...
const ImageGenerateToolSchema = Type.Object({
  ...
  model: Type.Optional(
    Type.String({
      description: 'Optional provider/model override, e.g. openai/gpt-image-1 or dashscope/wan2.6-t2i (DashScope Beijing).',
    }),
  ),
...
export function resolveImageGenerationModelConfigForTool(params: { cfg?: Config }): ToolModelConfig | null {
  ...
  return buildToolModelConfigFromCandidates({
    explicit,
    candidates: [`openai/${OPENAI_DEFAULT_IMAGE_MODEL}`, `dashscope/${DASHSCOPE_DEFAULT_IMAGE_MODEL}`],
  });
}
