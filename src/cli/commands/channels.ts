...
// HTTP / programmatic handlers for `POST /api/setup/channels.telegram/<action>`.

registerSetupHandler({
  domain: 'channels.telegram',
  action: 'add',
  handler: async ({ configPath, fields, options }) => {
    const accountId = typeof fields.account === 'string' && fields.account.trim()
      ? fields.account.trim()
      : 'default';
    const enable = fields.enable === undefined ? true : Boolean(fields.enable);
    ...
    if (!token) {
      return {
        ...
        errors: [{ path: 'token', message: 'token is required (Telegram bot token from @BotFather)' }],
      };
    }
    if (!validateTelegramToken(token)) {
      return {
    ...
    return runSetupHeadless({
      ...
      mutator: {
        ...
        target: accountId,
        action: 'add',
        mutate: (cfg) => applyTelegramAccount(cfg, accountId, token, enable),
        resultValue: () => ({ channel: 'telegram', accountId, tokenMask: maskToken(token) }),
      },
    });
  },
});
  ...
  handler: async ({ configPath, fields, options }) => {
    ...
    return runSetupHeadless({
      ...
      mutator: {
        ...
        target: accountId,
        action: 'remove',
        mutate: (cfg) => removeChannelAccount(cfg, 'telegram', accountId),
      },
    });
  },
});
...
/**
 * Full channels configuration write — used by the web Channels settings panel.
 * Accepts the complete `{ channels, bindings }` blob and merges it into config,
 * providing zod validation + diff semantics consistent with all other setup handlers.
 */
registerSetupHandler({
  domain: 'channels',
  action: 'configure',
  handler: async ({ configPath, fields, options }) => {
    const channels = fields.channels && typeof fields.channels === 'object' ? fields.channels : undefined;
    const bindings = Array.isArray(fields.bindings) ? fields.bindings : undefined;
    if (!channels && !bindings) {
      return {
        ok: false,
        action: 'set',
        domain: 'channels',
        changedPaths: [],
        dryRun: options.dryRun,
        errors: [{ message: 'At least one of `channels` or `bindings` fields is required.' }],
      };
    }
    return runSetupHeadless({
      configPath,
      ...
      mutator: {
        ...
        mutate(cfg) {
          ...
          if (channels) patched.channels = channels;
          if (bindings) patched.bindings = bindings;
          return patched as Config;
        },
        resultValue: (cfg) => ({
          channels: (cfg as Record<string, unknown>).channels,
        }),
      },
    });
  },
...
registerSetupDomain({
  domain: 'channels.telegram',
  description: 'Configure Telegram bot accounts (botToken from @BotFather).',
  docs: 'https://xopcai.github.io/xopc/channels',
  storage: 'channels.telegram in ~/.xopc/xopc.json',
  actions: [
    {
      name: 'list',
      cli: 'xopc channels list [--json]',
      description: 'List all channels and accounts with status.',
    },
    {
      name: 'add',
      cli: 'xopc channels add telegram [--token <token>] [--account <id>] [--dry-run] [--json]',
      description: 'Add or update a Telegram bot account. Prompts securely if --token is omitted.',
      fields: ['token', 'account'],
    },
    {
      name: 'remove',
      cli: 'xopc channels remove telegram [--account <id>] [--dry-run] [--json]',
      description: 'Remove a Telegram bot account.',
      fields: ['account'],
    },
    {
      name: 'schema',
      cli: 'xopc channels schema [channel] [--json]',
      description: 'Print credential schemas for channels.',
    },
  ],
  fields: {
    token: {
      type: 'string',
      description: 'Telegram bot API token (e.g. `123456789:AAH...`).',
      required: true,
      secret: true,
      source: 'https://t.me/BotFather → /newbot',
    },
    account: {
      type: 'string',
      description: 'Account id to manage when running multiple bots. Defaults to "default".',
      default: 'default',
    },
  },
});
