...
it('finds both current formats, retaining complete skill trees and scope', async () => {
  skill(join(home, '.agents/skills/report'));
  skill(join(home, '.claude/skills/report'));
  const codex = await scanLocal({ source: 'codex', home });
  expect(codex.candidates[0]).toMatchObject({ name: 'report', shared: true, compatibility: 'compatible' });
  const claude = await scanLocal({ source: 'claude-code', home });
  expect(claude.candidates[0].files.map(f => f.path)).toEqual(['references/example.md', 'SKILL.md']);
});
it('blocks executable extensions and rejects malformed YAML without a fallback parser', async () => {
  skill(join(home, '.claude/skills/report'), 'context: fork\n');
  expect((await scanLocal({ source: 'claude-code', home })).candidates[0].compatibility).toBe('blocked');
  skill(join(home, '.claude/skills/report'), 'hooks: [\n');
  expect((await scanLocal({ source: 'claude-code', home })).candidates[0].compatibility).toBe('blocked');
});
it('redacts MCP secrets and preserves unsupported permission constraints as blockers', async () => {
  mkdirSync(join(home, '.codex'));
  writeFileSync(join(home, '.codex/config.toml'), '[mcp_servers.docs]\ncommand="node"\nargs=["server.js"]\nenv={ API_KEY="a-secret-value" }\nenabled_tools=["read"]');
  const scan = await scanLocal({ source: 'codex', home });
...
it('rejects links inside skill trees', async () => {
...
it('does not persist skill trees containing probable credentials', async () => {
  skill(join(home, '.claude/skills/report'));
  writeFileSync(join(home, '.claude/skills/report/.env'), 'PASSWORD=example');
  const item = (await scanLocal({ source: 'claude-code', home })).candidates[0];
  expect(item.compatibility).toBe('blocked');
  expect(item.files).toEqual([]);
});
it('blocks missing referenced resources and project-specific MCP scope', async () => {
  skill(join(home, '.claude/skills/report'));
  ...
  expect((await scanLocal({ source: 'claude-code', home, projectRoot })).candidates.find(c => c.kind === 'mcp')).toMatchObject({ scope: 'project', compatibility: 'blocked' });
});
it('preserves passive Codex metadata without treating it as execution policy', async () => {
  skill(join(home, '.agents/skills/report'), 'metadata:\n  short-description: Weekly reporting\ndisable-model-invocation: true\n');
  const candidate = (await scanLocal({ source: 'codex', home })).candidates[0];
...
it('retains both Claude project instruction locations and Codex override precedence', async () => {
  ...
  const rules = (await scanLocal({ source: 'claude-code', home, projectRoot, projectOnly: true })).candidates;
