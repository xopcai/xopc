...
function createEmptySessionManager(cwd: string, sessionKey: string): TuiReadonlySessionManager {
  return {
    getEntries: () => [],
    getBranch: () => [],
...
function createDefaultModelRegistry(): TuiModelRegistry {
  return {
...
function resolveCustomOverlayOptions(
...
function invokeCustomFactory<T>(
...
class WrappedAutocompleteProvider implements AutocompleteProvider {
  constructor(
...
function isTuiIdle(state: Partial<TuiState>): boolean {
  return (
...
function hasPendingTuiMessages(state: Partial<TuiState>): boolean {
  return (
...
function getTuiContextUsage(state: Partial<TuiState>): TuiContextUsage | undefined {
...
function getTuiModelInfo(state: Partial<TuiState>): TuiModelInfo | undefined {
  const sessionInfo = state.sessionInfo;
  if (!sessionInfo) return undefined;
...
function getTuiThinkingLevel(state: Partial<TuiState>): TuiThinkingLevel | undefined {
...
function getTuiReasoningLevel(state: Partial<TuiState>): TuiReasoningLevel | undefined {
...
function getTuiVerboseLevel(state: Partial<TuiState>): TuiVerboseLevel | undefined {
