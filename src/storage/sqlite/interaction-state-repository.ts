...
export function getInteractionState(conversationId: string, now = Date.now()): InteractionState | undefined {
  const row = getSqliteDatabase().prepare(
    `SELECT conversation_id, support_need, emotion_hypothesis, confidence, source,
            repair_status, repair_reason, expires_at, updated_at
     FROM interaction_states WHERE conversation_id = ? AND expires_at > ?`,
  ).get(conversationId, now) as Row | undefined;
...
export function setInteractionState(input: {
  ...
  runSqliteWriteTransaction((db) => {
    db.prepare(
      `INSERT INTO interaction_states (
        conversation_id, support_need, emotion_hypothesis, confidence, source,
        repair_status, repair_reason, expires_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  return getInteractionState(input.conversationId, now)!;
}
...
export function updateInteractionStateFromMessage(input: {
  ...
  const now = input.now ?? Date.now();
  const previous = getInteractionState(input.conversationId, now);
  ...
  const signal = previous?.repairStatus === 'needed' && inferred.repairStatus === 'none'
    ...
    : inferred;
  return setInteractionState({ conversationId: input.conversationId, signal, now });
}
