...
type EvidenceRow = {
  ...
  conversation_id: string | null; turn_id: string | null; message_id: string | null;
...
export function createContextEvidence(
  ...
  if (existing) {
    db.prepare(`UPDATE context_evidence SET
      source_run_id = ?, source_item_id = ?, conversation_id = ?, turn_id = ?, message_id = ?,
  ...
  db.prepare(`INSERT INTO context_evidence (
    ...
    source_item_id, conversation_id, turn_id, message_id, content_hash, retention_policy,
    ...
    input.sourceRunId ?? null, input.sourceItemId ?? null, input.conversationId ?? null,
