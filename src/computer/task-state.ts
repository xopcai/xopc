...
export type ComputerVerification = { status: 'satisfied' | 'not_met' | 'unavailable'; source: 'accessibility'; scope: 'expectation' };

/** Verify only caller-specified conditions against scoped native evidence, never model prose. */
export function verifyComputerExpectation(expectation: ComputerExpectation | undefined, observation?: ComputerObservation): ComputerVerification | undefined {
  ...
  try {
    ...
    if (expectation.kind === 'text') {
      ...
      return result(data.text.includes(expectation.text) ? 'satisfied' : 'not_met');
    }
    ...
    if (fields.length !== 1 || typeof fields[0].value !== 'string') return result('unavailable');
