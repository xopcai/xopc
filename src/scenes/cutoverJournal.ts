...
import { resolveSceneCutoverBindings, type SceneCutoverBindings } from './cutoverBindings.js';
import { createSceneCutoverSnapshot, verifySceneCutoverSnapshot } from './cutoverSnapshot.js';
...
export async function commitSceneCutover(input: {
  ...
  convert: (db: DatabaseSync, config: Readonly<Record<string, unknown>>, bindings: SceneCutoverBindings) => undefined;
  ...
  const snapshot = await createSceneCutoverSnapshot({ db: input.db, backupRoot: input.backupRoot, configPath });
  ...
  try {
    ...
    if (input.convert(input.db, config, bindings) !== undefined) throw new Error('Scene database conversion must be synchronous');
    assertConvertedDatabase(input.db);
    input.db.exec(`CREATE TABLE scene_cutover_journal (
