...
import { closeXopcDatabase, openXopcDatabase, resetXopcDatabaseSingletonForTest, upsertConnectorConnection, upsertKnowledgeSourceItems } from '../../storage/sqlite/index.js';
...
describe('mail scene backend vertical slice', () => {
  it('starts, checks a real synchronized thread, and publishes one draft without calling a write tool', async () => {
    ...
    try {
      ...
      const connection = upsertConnectorConnection({ id: 'mail', connectorId: 'gmail', provider: 'composio', principalId: principal.ownerId,
        providerConnectionId: 'test-mail', identity: {}, status: 'active', isDefault: true, metadata: {} });
