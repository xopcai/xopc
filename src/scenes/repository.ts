...
export class SceneRepository {
  ...
  listRuns(principal: ScenePrincipal, activationId: string, limit = 50, afterId = '') {
    ...
    return this.db.prepare(`SELECT id, activation_id AS activationId, status, attempt, reason, created_at AS createdAt, retry_at AS retryAt
      FROM scene_runs WHERE activation_id = ? AND id > ? ORDER BY id LIMIT ?`).all(activationId, afterId, limit);
  }
  ...
  listInbox(principal: ScenePrincipal, limit = 50, afterId = '', activationId: string | null = null): Array<{ id: string; activationId: string; outcomeId: string; content: unknown; status: string }> {
    ...
    const rows = this.db.prepare(`SELECT p.id, p.status, o.id AS outcome_id, o.content_json, a.id AS activation_id
      ...
      AND (? IS NULL OR a.id = ?)
      AND p.status <> 'withdrawn' AND (? IS NULL OR p.created_at < ? OR (p.created_at = ? AND p.id < ?))
      ORDER BY p.created_at DESC, p.id DESC LIMIT ?`)
      .all(principal.ownerId, principal.workspaceId, activationId, activationId, cursor?.created_at ?? null, cursor?.created_at ?? null, cursor?.created_at ?? null, afterId, limit) as Row[];
