...
import type { SceneActivation, ScenePrincipal } from './contracts.js';
...
export class SceneMailContextProvider implements SceneContextProvider {
  ...
  authorizedAccounts(activation: SceneActivation): string[] {
    ...
    const row = this.db.prepare(`SELECT a.id FROM knowledge_source_items k
      ...
      ${accountJoin}
      WHERE k.item_id = ? AND k.item_type = 'email' AND k.deleted_at IS NULL
        AND k.sensitivity NOT IN ('secret', 'regulated')
        AND json_valid(k.metadata_json) AND json_valid(k.normalized_text)
        AND json_type(k.normalized_text, '$.threadId') = 'text'
        AND length(trim(json_extract(k.normalized_text, '$.threadId'))) > 0
      ...
      .get(activation.scope.ids[0], activation.workspaceId, activation.ownerId, this.clock() / 1000);
    return row && activation.permissions.accountIds.includes(String(row.id)) ? [String(row.id)] : [];
  }
