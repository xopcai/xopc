  ...
  return (
    <section id="project-panel-skills" role="tabpanel" aria-labelledby="project-primary-tab-skills" className="grid content-start gap-4">
      ...
      <Dialog.Root open={installMode !== null} onOpenChange={(open) => { if (!open) setInstallMode(null); }}>
        <Dialog.Portal>
          ...
          <Dialog.Content className={cn(
            'xopc-dialog-content fixed left-1/2 top-1/2 z-50 flex -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-edge bg-surface-overlay shadow-xl',
      ...
      <Dialog.Root open={Boolean(preview)} onOpenChange={(open) => { if (!open) setPreview(null); }}>
        <Dialog.Portal>
          <Dialog.Overlay className="xopc-dialog-overlay fixed inset-0 z-50 bg-scrim backdrop-blur-[2px]" />
          <Dialog.Content className="xopc-dialog-content fixed left-1/2 top-1/2 z-50 flex h-[min(42rem,calc(100vh-2rem))] w-[min(48rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-edge bg-surface-overlay shadow-xl">
