...
export function AutomationsWorkspace({
  ...
  const filter: AutomationFilter = filterParam === 'active' || filterParam === 'paused' || filterParam === 'system' || filterParam === 'attention' ? filterParam : 'all';
  ...
  const selectedRun = useMemo(
    () => runs.find((run) => run.id === selectedRunId) ?? selectedRunsSwr.data?.runs.find((run) => run.id === selectedRunId) ?? null,
    [runs, selectedRunId, selectedRunsSwr.data],
  );
  ...
  const selectedAutomationRuns = useMemo(
    () => [...(selectedRunsSwr.data?.runs ?? runs)]
      .filter((run) => run.automationId === selectedAutomationId)
      .sort((a, b) => b.createdAtMs - a.createdAtMs),
    [runs, selectedAutomationId, selectedRunsSwr.data],
  );
  ...
  const filteredAutomations = useMemo(() => (filter === 'system' ? systemAutomations : userAutomations).filter((automation) => {
    if (filter === 'attention' && !automationHasExecutionIssue(automation, userRuns)) return false;
  ...
  }, [automationParam, filteredAutomations, selectedAutomationId]);

  const reload = useCallback(async () => {
    await Promise.all([automationsSwr.mutate(), runsSwr.mutate(), selectedRunsSwr.mutate(), runEventsSwr.mutate()]);
  }, [automationsSwr.mutate, runEventsSwr.mutate, runsSwr.mutate, selectedRunsSwr.mutate]);
  ...
  useEffect(() => {
    ...
    if (!automation || !isSystemManagedAutomation(automation) || filter === 'system') return;
    setSearchParams((previous) => { const next = new URLSearchParams(previous); next.set('status', 'system'); return next; }, { replace: true });
  }, [automationParam, automations, filter, setSearchParams]);
  ...
  const openAutomationDetails = useCallback((automationId: string) => {
    ...
    const latestRun = latestAutomationRun(automationId, runs);
  ...
  }, [markRunRead, runs, setSearchParams]);
  ...
  return (
    <div className={cn('flex min-h-0 min-w-0 flex-1 flex-col', embedded ? 'bg-transparent' : 'bg-surface-panel')}>
      <div className={cn('flex w-full flex-col gap-4', embedded ? 'py-1' : 'mx-auto max-w-[96rem] px-4 py-7 sm:px-6 lg:px-8 lg:py-9')}>
        ...
        {error || automationsSwr.error || runsSwr.error ? (
        ...
        {!initialLoading && !automationsSwr.error && userAutomations.length === 0 && filter !== 'system' ? (
          <section className="rounded-xl border border-edge-subtle bg-surface-base p-6">
            <h2 className="text-lg font-semibold text-fg">{labels.experience.scenes}</h2>
        ...
        ) : userAutomations.length > 0 || filter === 'system' ? (
          <section className="grid min-h-0 items-start gap-4 xl:grid-cols-[minmax(18rem,0.8fr)_minmax(0,1.5fr)]">
            <aside className="overflow-hidden rounded-xl border border-edge-subtle bg-surface-base shadow-surface">
              ...
              <AutomationList
                ...
                runs={filter === 'system' ? runs : userRuns}
                labels={labels}
            ...
            {selectedAutomation ? (
              ...
              <EmptyState icon={<Zap className="size-5" />} title={labels.empty.filtered} />
            )}
          </section>
        ) : null}
        ...
        return (
          ...
          >
            ...
            <div className="mt-2 space-y-1 pl-4 text-xs text-fg-muted">
              ...
              <div>{automationTriggerLabel(automation.trigger, labels, cronLabels, language)}</div>
            </div>
          </button>
        );
      })}
