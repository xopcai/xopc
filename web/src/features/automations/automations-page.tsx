...
type AutomationTemplate = { name: string; description: string; form: FormState };
...
function automationNeedsAttention(automation: Automation, runs: AutomationRun[]): boolean {
  if ((automation.state.consecutiveFailures ?? 0) > 0) return true;
  const latestRun = runs.reduce<AutomationRun | undefined>((latest, run) => {
    if (run.automationId !== automation.id) return latest;
    return !latest || run.createdAtMs > latest.createdAtMs ? run : latest;
  }, undefined);
  return latestRun ? needsAttention(latestRun) : false;
}
...
export function AutomationsPage() {
  ...
  useEffect(() => {
    ...
    setFilter(isSystemManagedAutomation(automation) ? 'system' : 'all');
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-surface-panel">
      <div className="flex w-full flex-col gap-4 px-3 py-5 sm:px-5 xl:px-6">
        {error ? (
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {templates.slice(0, 4).map((template) => (
              <button
                type="button"
                key={template.name}
                className="rounded-lg border border-edge-subtle bg-surface-panel p-3 text-left hover:bg-surface-hover"
                onClick={() => onTemplateSelect(template.form)}
              >
          ...
          >
            <div className="flex min-w-0 items-center gap-2">
              <span className={cn(
                ...
                activeRun
                  ? 'bg-blue-500'
                  : automationNeedsAttention(automation, recentRuns)
                    ? 'bg-red-500'
                    : automation.enabled
                      ? 'bg-emerald-500'
                      : 'bg-fg-subtle',
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.Content align="end" className="z-70 min-w-40 rounded-lg border border-edge bg-surface-panel p-1 shadow-popover">
                  ...
                  <DropdownMenu.Item
                    ...
                    disabled={busyAction !== null}
                    onSelect={() => void onAction(`automation:${automation.id}:delete`, () => automationApi.remove(automation.id), labels.dashboard.deleted)}
                  >
