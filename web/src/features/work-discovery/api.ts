...
export async function updateWorkUnderstandingThread(
  threadId: string,
  input: {
...
): Promise<WorkUnderstandingThread> {
  const response = await fetchJson<{ thread: WorkUnderstandingThread }>(
    apiUrl(`/api/work-understanding/threads/${encodeURIComponent(threadId)}`),
