    ...
    >
      {form ? <>
        ...
        >
          ...
          {form.driverKind === 'playwright' ? <div className="space-y-4 rounded-lg bg-surface-inset p-4">
          ...
          {form.driverKind === 'cdp' ? <div className="rounded-lg bg-surface-inset p-4"><TextField label="CDP endpoint" description={zh ? '例如 http://127.0.0.1:9222。Chrome 需要开启 remote debugging。' : 'For example http://127.0.0.1:9222. Chrome must be started with remote debugging enabled.'} value={form.cdpEndpoint} onChange={(cdpEndpoint) => vm.update({ cdpEndpoint })} /></div> : null}
          {form.driverKind === 'remote' ? <div className="space-y-4 rounded-lg bg-surface-inset p-4">
...
function TextField({ label, description, value, onChange, type = 'text', multiline = false, autoComplete }: { label: string; description?: string; value: string; onChange: (value: string) => void; type?: string; multiline?: boolean; autoComplete?: string }) { const className = 'w-full rounded-lg border border-edge bg-surface-inset px-3 py-2 text-sm text-fg outline-none focus:border-accent'; return <Field label={label} description={description}>{multiline ? <textarea rows={3} value={value} onChange={(event) => onChange(event.target.value)} className={className} /> : <input type={type} autoComplete={autoComplete} value={value} onChange={(event) => onChange(event.target.value)} className={className} />}</Field>; }
function NumberField({ label, description, value, onChange, min, max, step = 1 }: { label: string; description?: string; value: number; onChange: (value: number) => void; min: number; max: number; step?: number }) { return <Field label={label} description={description}><input type="number" min={min} max={max} step={step} value={value} onChange={(event) => { const next = Number(event.target.value); if (Number.isFinite(next)) onChange(next); }} className="h-10 w-full rounded-lg border border-edge bg-surface-inset px-3 text-sm text-fg outline-none focus:border-accent" /></Field>; }
