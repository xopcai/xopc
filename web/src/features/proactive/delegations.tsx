...
import { checkOutcome, formatAssistantDate, delegationNextTrigger, delegationState, delegationTitle, latestCardSummary } from './presentation';
...
export function DelegationListItem({ sub, card, zh }: { sub: Delegation; card?: ProactiveCard; zh: boolean }) {
  return <article className="rounded-2xl border border-edge bg-surface-panel p-5">
    ...
    <div className="mt-4 rounded-xl bg-surface-hover px-4 py-3 text-sm"><p className="font-medium text-fg">{delegationState(sub, zh, card)}</p><p className="mt-1 text-fg-muted">{latestCardSummary(card, zh)}</p><p className="mt-1 text-fg-muted">{sub.checkStatus.lastCheckedAt && `${zh ? '最近检查' : 'Last checked'}：${formatAssistantDate(sub.checkStatus.lastCheckedAt, zh)} · `}{delegationNextTrigger(sub, zh)}</p></div>
  </article>;
}
...
export function DelegationDetail({ sub, card, zh, refresh }: { sub: Delegation; card?: ProactiveCard; zh: boolean; refresh: () => void }) {
  ...
  const [interval, setInterval] = useState(sub.scanIntervalMinutes);
  const [draftRevision, setDraftRevision] = useState(sub.revision);
  ...
  async function change(patch: Record<string, unknown>, check = false) {
    ...
    try {
      await proactiveWrite(`/api/proactive/subscriptions/${encodeURIComponent(sub.id)}${check ? '/check' : ''}`, check ? 'POST' : 'PATCH', check ? {} : { ...patch, expectedRevision: editing ? draftRevision : sub.revision });
      setEditing(false);
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-edge p-5"><h2 className="font-semibold">{zh ? '最近检查' : 'Recent checks'}</h2>{sub.checkStatus.recent.length === 0 ? <p className="mt-2 text-sm text-fg-muted">{zh ? '尚无分析记录' : 'No analysis yet'}</p> : sub.checkStatus.recent.map(run => <p key={run.id} className="mt-2 text-sm text-fg-muted">{formatAssistantDate(run.startedAt, zh)} · {checkOutcome(run.outcome, run.status, zh)}{run.retryAt && ` · ${zh ? '重试' : 'Retry'} ${formatAssistantDate(run.retryAt, zh)}`}</p>)}</section>
    ...
    <section className="rounded-2xl border border-edge bg-surface-panel p-5">
      ...
      {editing && <div className="mt-5 border-t border-edge pt-5">{sub.scopeKind === 'project' && <label className="block text-sm">{zh ? '检查间隔（分钟）' : 'Check interval (minutes)'}<input className={field} type="number" min={15} max={10080} value={interval} onChange={event => setInterval(Number(event.target.value))} /></label>}<label className="block text-sm">{zh ? '希望助理关注什么，忽略什么？' : 'What should the assistant follow or ignore?'}<textarea rows={5} maxLength={12000} className={field} value={instructions} onChange={event => setInstructions(event.target.value)} /></label><label className="mt-4 block text-sm">{zh ? '什么时候提醒我' : 'When to notify me'}<Select value={delivery} onChange={event => setDelivery(event.target.value as typeof delivery)}><SelectOption value="inbox">{zh ? '回到工作台时告诉我' : 'Tell me in the Workbench'}</SelectOption><SelectOption value="important">{zh ? '重要变化立即告诉我' : 'Tell me right away for important changes'}</SelectOption><SelectOption value="digest">{zh ? '放进每日汇总' : 'Include it in my daily digest'}</SelectOption></Select></label></div>}
      {!sub.completedAt && <div className="mt-5 flex flex-wrap gap-2">{editing ? <><Button disabled={busy || !instructions.trim() || draftRevision !== sub.revision || (sub.scopeKind === 'project' && (!Number.isInteger(interval) || interval < 15 || interval > 10080))} onClick={() => void change({ userInstructions: instructions, delivery, ...(sub.scopeKind === 'project' ? { scanIntervalMinutes: interval } : {}) })}>{zh ? '保存安排' : 'Save arrangement'}</Button><Button variant="ghost" onClick={() => setEditing(false)}>{zh ? '取消' : 'Cancel'}</Button></> : <Button variant="ghost" onClick={() => { setInstructions(sub.userInstructions); setDelivery(sub.delivery); setInterval(sub.scanIntervalMinutes); setDraftRevision(sub.revision); setEditing(true); }}>{zh ? '调整安排' : 'Adjust arrangement'}</Button>}<Button variant="ghost" disabled={busy} onClick={() => void change({ enabled: false, completedAt: new Date().toISOString() })}>{zh ? '结束安排' : 'End arrangement'}</Button></div>}
    ...
    {editing && draftRevision !== sub.revision && <p role="alert" className="text-sm text-danger">{zh ? '安排已更新，请取消编辑后重新载入。' : 'This arrangement changed. Cancel editing and reload.'}</p>}
