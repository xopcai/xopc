...
export function ProactiveToday({ compact = false }: { compact?: boolean }) {
  ...
  if (!scenes.length) return <section className="rounded-2xl border border-edge bg-surface-panel p-6">
    <h2 className="font-medium text-fg">{zh ? '今天没有需要你接手的变化' : 'Nothing needs your attention today'}</h2>
    <p className="mt-2 text-sm text-fg-muted">{zh ? '继续工作即可。需要持续关注的事，可以直接在项目或对话里交给助理。' : 'Keep working. You can ask the assistant to follow something from its project or conversation.'}</p>
  </section>;
  ...
  return <section aria-labelledby="proactive-scenes-title">
    <div className="mb-3 flex items-center justify-between gap-3 px-1">
      ...
      <Link className="text-xs text-fg-subtle hover:text-accent" to="/assistant-work">{zh ? '助理安排' : 'Assistant arrangements'}</Link>
    </div>
