...
export function ProactiveCardView({ card, copy, refresh, detail = false }: { card: ProactiveCard; copy: ProactiveCopy; refresh: () => void; detail?: boolean }) {
  ...
  const actionable = !['withdrawn', 'expired', 'resolved'].includes(card.status);
  ...
  async function act(actionId: ProactiveCardAction['actionId'], extra: Partial<ProactiveCardAction> = {}) {
    ...
    try {
      await proactiveWrite(`/api/inbox/judgments/${encodeURIComponent(card.id)}/actions`, 'POST', { ...extra, actionId, expectedRevision: card.revision, idempotencyKey: crypto.randomUUID() });
  ...
  return <article className="rounded-2xl border border-edge bg-surface-panel p-5">
    ...
    {!detail && <Link className="mt-4 inline-block text-sm font-medium text-accent" to={`/proactive?item=${encodeURIComponent(card.id)}`}>{card.decision ? (zh ? '查看并决定' : 'Review decision') : card.artifact ? (zh ? '打开准备好的成果' : 'Open prepared work') : (zh ? '查看变化' : 'View update')}</Link>}
    {detail && card.artifact && <section className="mt-5 border-t border-edge pt-5">
      ...
      {actionable && <div className="mt-3 flex gap-2">{editing ? <><Button disabled={busy || !content.trim()} onClick={() => void act('edit_artifact', { artifact: { ...card.artifact!, content } })}>{copy.save}</Button><Button variant="ghost" onClick={() => { setEditing(false); setContent(card.artifact!.content); }}>{zh ? '取消' : 'Cancel'}</Button></> : <Button variant="ghost" onClick={() => setEditing(true)}>{zh ? '修改成果' : 'Edit'}</Button>}</div>}
    </section>}
    {detail && card.decision && actionable && <section className="mt-5 rounded-xl border border-edge p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">{card.decision.options.map((option, i) => <Button key={option.id} variant={i === 0 ? 'primary' : 'ghost'} disabled={busy || (option.id === 'approve' && taskDraft !== undefined && (!taskDraft.title.trim() || !taskDraft.objective.trim()))} onClick={() => void act('decide', { choice: option.id, ...(option.id === 'approve' && taskDraft ? { taskDraft } : {}) })}>{option.label}</Button>)}</div>
    </section>}
    ...
    {card.actionStatus === 'failed' && !['expired', 'withdrawn'].includes(card.status) && <div className="mt-3 text-sm text-danger">{card.actionError}<Button disabled={busy} onClick={() => void act('retry')}>{copy.retry}</Button></div>}
    ...
    {detail && actionable && <details className="mt-4 text-sm"><summary className="cursor-pointer text-fg-muted">{zh ? '调整或收起' : 'Adjust or dismiss'}</summary><div className="mt-3 flex flex-wrap gap-2"><Button disabled={busy} onClick={() => void act('handled')}>{zh ? '我已处理' : 'Already handled'}</Button><Button disabled={busy} onClick={() => void act('resolve')}>{zh ? '这条不用' : 'Dismiss'}</Button><Button disabled={busy} onClick={() => void act('snooze')}>{copy.snooze}</Button><Link className="p-2 text-accent" to={`/proactive?tab=delegations&subscription=${encodeURIComponent(card.subscriptionId)}`}>{zh ? '修改关注要求' : 'Adjust instructions'}</Link></div></details>}
