...
export function ProactiveCardView({ card, copy, refresh, detail = false }: { card: ProactiveCard; copy: ProactiveCopy; refresh: () => void; detail?: boolean }) {
  ...
  return <article className="rounded-2xl border border-edge bg-surface-panel p-5">
    ...
    {detail && actionable && <details className="mt-4 text-sm"><summary className="cursor-pointer text-fg-muted">{zh ? '调整或收起' : 'Adjust or dismiss'}</summary><div className="mt-3 flex flex-wrap gap-2"><Button disabled={busy} onClick={() => void act('handled')}>{zh ? '我已处理' : 'Already handled'}</Button><Button disabled={busy} onClick={() => void act('resolve')}>{zh ? '这条不用' : 'Dismiss'}</Button><Button disabled={busy} onClick={() => void act('snooze')}>{copy.snooze}</Button><Link className="p-2 text-accent" to={card.communication ? `/assistant-work?follow-up=${encodeURIComponent(card.communication.id)}` : `/assistant-work?delegation=${encodeURIComponent(card.subscriptionId)}`}>{zh ? '修改关注要求' : 'Adjust instructions'}</Link></div></details>}
