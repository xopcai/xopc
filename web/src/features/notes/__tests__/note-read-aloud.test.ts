...
describe('note full-text reading', () => {
  it('reads the title and every paragraph without attachment addresses or checklist syntax', () => {
    const text = buildNoteReadAloudText('旅行计划', [
      '## 第一天', '- [x] 到达机场。',
      '[行程](https://example.com)和![地图](xopc-attachment://notes/n1/a1)',
      '最后一天回家。',
    ].join('\n'));
    for (const part of ['旅行计划', '第一天', '到达机场。', '行程和地图', '最后一天回家。']) expect(text).toContain(part);
    expect(text).not.toMatch(/https?:|xopc-attachment:|\[x\]/);
  });
