...
export function OutcomeCard({ item, zh, onChange }: { item: SceneOutcome; zh: boolean; onChange: () => void }) {
  ...
  return <article ref={card} className="space-y-3 rounded-xl border border-edge bg-surface-panel p-4 sm:p-6">
    ...
    {item.sourceHealth?.reason && <p role="status" className="text-sm text-fg-muted">{zh ? '邮件来源暂不可用，这份成果尚未核实最新邮件，请检查连接授权后重试。' : 'Mail is unavailable. This result has not been checked against the latest messages. Check connection access and retry.'}</p>}
    ...
    <div className="space-y-1 text-xs text-fg-muted">{item.sources?.length ? item.sources.map(source => <p key={source.href}><a className="text-accent underline" href={source.href} target={source.kind === 'mail' ? '_blank' : undefined} rel="noreferrer">{source.kind === 'notes' ? (zh ? '查看关注资料' : 'View monitor notes') : source.title || (zh ? '查看原邮件' : 'View original email')}</a>{source.sender && ` · ${source.sender}`}</p>) : <p>{zh ? '来源详情暂不可用' : 'Source details unavailable'}</p>}</div>
