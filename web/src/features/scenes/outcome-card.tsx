...
export function OutcomeCard({ item, zh, onChange }: { item: SceneOutcome; zh: boolean; onChange: () => void }) {
  const path = `/presentations/${item.id}`;
  const feedback = useSWR<{ feedback: Feedback | null }>(`${path}/feedback`, sceneGet);
  ...
  const rate = async (rating: Feedback['rating']) => {
    ...
    try {
      ...
      const result = await sceneWrite<{ revision: number }>(`${path}/feedback`, 'POST', { expectedRevision: feedback.data.feedback?.revision ?? 0, rating, note });
  ...
  return <article className="space-y-3 rounded-xl border border-edge bg-surface-panel p-4 sm:p-6">
    ...
    {feedback.error ? <p role="alert" className="text-sm text-danger">{sceneErrorText(feedback.error, zh)}<Button onClick={() => void feedback.mutate()}>{zh ? '重新加载' : 'Reload'}</Button></p>
