...
export function SceneDiagnostics({ activationId, zh }: { activationId: string; zh: boolean }) {
  ...
  const reasons: Record<string, [string, string]> = {
    ...
    source_not_ready: ['来源暂不可用，请检查连接器授权；稍后会重试。', 'Source unavailable. Check connector access; a retry is scheduled.'],
    ...
    invalid_model_result: ['模型返回的格式不符合此场景要求，结果未发布；请重试或更换模型。', 'The model returned an unsupported result. Nothing was published; retry or choose another model.'],
