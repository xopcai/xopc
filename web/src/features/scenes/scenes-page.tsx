import { ArrowLeft, CalendarDays, CirclePause, Inbox, RefreshCw, Sparkles } from 'lucide-react';
import { useCallback, useEffect, useRef, useState, type FormEvent } from 'react';
import { Link, useBeforeUnload, useBlocker, useMatch, useNavigate, useParams, useSearchParams } from 'react-router-dom';
...
import { ImportedSceneContext } from './imported-context';
...
function SceneContent() {
  ...
  return <main className="h-full overflow-y-auto bg-surface-panel p-4 sm:p-8"><div className="mx-auto max-w-3xl space-y-6">
    {inbox ? <SceneInbox /> : templateKey ? <CreateScene key={templateKey} templateKey={templateKey} /> : activationId ? <SceneDetail key={activationId} id={activationId} /> : <SceneList />}
  </div></main>;
}
...
function SceneList() {
  ...
  return <>
    <header className="space-y-2"><h1 className="text-balance text-xl font-semibold tracking-tight text-fg">{zh ? '场景' : 'Scenes'}</h1>
      <p className="text-sm text-fg-muted">{zh ? '把需要持续留意的事交给 AI，把时间留给自己。' : 'Let AI look after recurring needs, leaving more time for you.'}</p>
      <Button asChild><Link to="/scenes/inbox"><Inbox size={16} aria-hidden="true" />{zh ? '查看全部成果' : 'View all results'}</Link></Button></header>
...
function FeedbackOverview({ zh }: { zh: boolean }) {
  ...
  return <section className={`${panelClass} space-y-3`} aria-label={zh ? '反馈概览' : 'Feedback overview'}>
    ...
    <p className="text-sm text-fg-muted">{zh ? '按你这段时间的明确反馈统计，每份成果取最新评价。已读、检查成功和导入的历史反馈不计入，也不代表已采纳或节省时间。' : 'Counts your explicit feedback during this period, using the latest rating per result. Reads, successful checks and imported feedback do not count. This does not measure adoption or time saved.'}</p>
  </section>;
}
...
function SceneInbox() {
  ...
  if (results.error) return <><Back zh={zh} /><Failure error={results.error} retry={() => void results.mutate()} /></>;
  if (!results.data) return <><Back zh={zh} /><Loading /></>;
  return <><Back zh={zh} /><header className="space-y-2"><h1 className="text-balance text-xl font-semibold tracking-tight text-fg">{digestId ? (zh ? '摘要中的成果' : 'Results in this digest') : (zh ? '场景成果' : 'Scene results')}</h1>
    <p className="text-sm text-fg-muted">{zh ? '集中查看各个场景准备的成果，告诉 AI 哪些对你有帮助。' : 'Review results across your scenes and tell AI what helps.'}</p></header>
    ...
    <section className="space-y-4" aria-label={zh ? '成果列表' : 'Results list'}>
      {results.data.outcomes.length ? results.data.outcomes.map((item) => <div key={item.id} className="space-y-2">
        ...
        <Button asChild variant="ghost"><Link to={`/scenes/${encodeURIComponent(item.activationId)}`}>{zh ? '查看所属场景' : 'View scene'}</Link></Button>
...
function SceneDetail({ id }: { id: string }) {
  ...
  const historyOnly = activation.templateKey === 'imported-history';
  return <><Back zh={zh} /><header className="space-y-3"><h1 className="break-words text-balance text-xl font-semibold text-fg">{activation.goal}</h1>
    <p className="text-sm text-fg-muted">{historyOnly ? (zh ? '迁移前的历史记录' : 'History from before the upgrade') : statusText(activation.status, zh)}</p>
    <p className="text-sm text-fg-muted">{historyOnly ? (zh ? '原有说明与成果已保留。选择当前支持的场景并确认范围后，即可重新开始。' : 'Your instructions and results are preserved. Choose an available scene and confirm its scope to start again.') : (zh ? '只准备建议和成果，不自动发送或写入外部系统。' : 'Prepares suggestions and results. Does not send or write to external systems.')}</p>
    {historyOnly ? <Button asChild><Link to="/scenes">{zh ? '选择新场景' : 'Choose a new scene'}</Link></Button> : <div className="flex flex-wrap gap-3"><Button variant="primary" disabled={busy || activation.status !== 'active'} onClick={() => void act(true)}>{busy ? (zh ? '处理中…' : 'Working…') : (zh ? '现在检查' : 'Check now')}</Button>
    ...
    {historyOnly && <ImportedSceneContext activationId={id} zh={zh} />}
    {!historyOnly && activation.permissions.contextProviders.includes('user_notes') && <NotesEditor path={path} zh={zh} onDirty={setNotesDirty} onSaved={() => { void results.mutate(); void runs.mutate(); }} />}
    {!historyOnly && (activation.templateKey === 'mail-follow-up' ? <MailDeadlineEditor activation={activation} zh={zh} onDirty={setScheduleDirty} onChanged={() => { void results.mutate(); void runs.mutate(); }} /> : <ScheduleEditor activation={activation} zh={zh} onDirty={setScheduleDirty} />)}
    ...
    <section className="space-y-3"><h2 className="text-base font-semibold text-fg">{zh ? '最近成果' : 'Recent results'}</h2>
      {results.error ? <Failure error={results.error} retry={() => void results.mutate()} /> : !results.data ? <Loading /> : results.data.outcomes.filter((item) => item.activationId === id).length === 0
        ? <p className="text-sm text-fg-muted">{historyOnly ? (zh ? '没有可展示的历史成果。' : 'No historical results to display.') : (zh ? '还没有新成果。提供资料后，可以先检查一次。' : 'No new results yet. Provide context and try a check.')}</p>
    ...
    <section className="space-y-3"><h2 className="text-base font-semibold text-fg">{zh ? '检查记录' : 'Check history'}</h2>
      {runs.error ? <Failure error={runs.error} retry={() => void runs.mutate()} /> : !runs.data ? <Loading /> : runs.data.runs.length === 0 ? <p className="text-sm text-fg-muted">{zh ? '尚未执行检查。' : 'No checks yet.'}</p>
        : <ul className="divide-y divide-edge">{runs.data.runs.map((run) => <li key={run.id} className="flex flex-wrap justify-between gap-2 py-3 text-sm"><span className="text-fg">{statusText(run.status, zh)}{run.origin === 'import' ? (zh ? ' · 历史记录' : ' · Imported history') : ''}{run.reason === 'empty_input' ? (zh ? '：请先提供资料' : ': provide context first') : ''}</span><time className="text-fg-muted" dateTime={new Date(run.createdAt).toISOString()}>{new Intl.DateTimeFormat(zh ? 'zh-CN' : 'en', { dateStyle: 'medium', timeStyle: 'short' }).format(run.createdAt)}</time></li>)}</ul>}
