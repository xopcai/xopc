...
export function BrowserAutomationsPage() {
  ...
  const [automations, setAutomations] = useState<BrowserAutomation[] | null>(null);
  const [selectedId, setSelectedId] = useState<string>();
  const [inputs, setInputs] = useState<Record<string, unknown>>({});
  const [runs, setRuns] = useState<BrowserAutomationRun[]>([]);
  const [activeRun, setActiveRun] = useState<BrowserAutomationRun>();
  const [message, setMessage] = useState('');
  const [busy, setBusy] = useState(false);
  const selected = useMemo(() => automations?.find((automation) => automation.id === selectedId), [selectedId, automations]);

  const loadAutomations = useCallback(async () => {
    const data = await browserAutomationApi.list();
    setAutomations(data.automations);
    setSelectedId((current) => current && data.automations.some((automation) => automation.id === current) ? current : data.automations[0]?.id);
  }, []);

  useEffect(() => { void loadAutomations().catch((error) => setMessage(String(error))); }, [loadAutomations]);
  useEffect(() => {
    if (!selected) return;
    setInputs(defaultBrowserAutomationInputs(selected));
    setActiveRun(undefined);
    setMessage('');
    void browserAutomationApi.listRuns(selected.id).then((data) => setRuns(data.runs)).catch((error) => setMessage(String(error)));
  }, [selected]);
  useEffect(() => {
    if (!activeRun || isTerminal(activeRun.status)) return;
    const timer = window.setInterval(() => {
      void browserAutomationApi.getRun(activeRun.id).then(({ run }) => {
        setActiveRun(run);
        if (isTerminal(run.status) && selected) void browserAutomationApi.listRuns(selected.id).then((data) => setRuns(data.runs));
  ...
  const setEnabled = async () => {
    ...
    setBusy(true);
    try {
      await browserAutomationApi.setEnabled(selected.id, !selected.enabled);
      await loadAutomations();
    } catch (error) { setMessage(String(error)); } finally { setBusy(false); }
  };
  const run = async () => {
    if (!selected) return;
    if (!browserAutomationInputsComplete(selected, inputs)) { setMessage(text.required); return; }
    setBusy(true);
    try {
      const { run: nextRun } = await browserAutomationApi.run(selected.id, inputs);
  ...
  const remove = async () => {
    ...
    setBusy(true);
    try {
      await browserAutomationApi.remove(selected.id);
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-surface-panel p-4 sm:p-6">
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        {selected && <main className="min-h-0 overflow-y-auto rounded-xl border border-edge bg-surface-base p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><h2 className="text-lg font-semibold text-fg">{selected.name}</h2>{selected.description && <p className="mt-1 max-w-2xl text-sm leading-6 text-fg-muted">{selected.description}</p>}<div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-surface-inset px-2.5 py-1 text-fg-muted">{text[selected.risk]}</span>{selected.domains.map((domain) => <span key={domain} className="flex items-center gap-1 rounded-full bg-surface-inset px-2.5 py-1 text-fg-muted"><Globe2 className="size-3" />{domain}</span>)}</div></div><div className="flex gap-2"><Button variant="secondary" disabled={busy} onClick={() => void setEnabled()}>{selected.enabled ? <CirclePause className="size-4" /> : <CirclePlay className="size-4" />}{selected.enabled ? text.pause : text.enable}</Button><Button variant="secondary" onClick={() => openAssistant(text.editPrompt.replace('{name}', selected.name).replace('{id}', selected.id))}><Pencil className="size-4" />{text.edit}</Button></div></div>
          <section className="mt-7 max-w-xl"><h3 className="text-sm font-semibold text-fg">{text.inputs}</h3>{Object.keys(selected.inputs).length === 0 ? <p className="mt-2 text-sm text-fg-muted">{text.noInputs}</p> : <div className="mt-3"><BrowserAutomationInputFields automation={selected} values={inputs} language={language} onChange={setInputs} /></div>}<div className="mt-5 flex gap-2"><Button variant="primary" disabled={!selected.enabled || busy || Boolean(activeRun && !isTerminal(activeRun.status))} onClick={() => void run()}><Play className="size-4" />{text.run}</Button>{activeRun && !isTerminal(activeRun.status) && <Button variant="secondary" onClick={() => void browserAutomationApi.cancel(activeRun.id)}><Square className="size-4" />{text.stop}</Button>}</div>{message && <p className="mt-3 text-sm text-red-700 dark:text-red-300">{message}</p>}</section>
          {activeRun && <section className="mt-7 rounded-xl border border-edge bg-surface-inset p-4"><div className="flex items-center gap-2 text-sm font-medium text-fg"><Clock3 className="size-4" />{text[activeRun.status]}</div>{activeRun.error && <p className="mt-2 text-sm text-red-700 dark:text-red-300">{activeRun.error}</p>}{activeRun.status === 'succeeded' && activeRun.result !== undefined && <div className="mt-3"><h3 className="text-xs font-semibold uppercase tracking-wide text-fg-muted">{text.result}</h3><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap text-sm text-fg">{formatResult(activeRun.result)}</pre></div>}</section>}
