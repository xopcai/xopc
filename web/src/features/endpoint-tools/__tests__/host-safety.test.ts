...
function fixture(focused = true) {
  ...
  return { execute, host: internal, sent, channel };
}
...
describe('EndpointToolHost execution guards', () => {
  ...
  it('does not execute an expired invocation', async () => {
    const { execute, host, channel } = fixture();
    await host.invoke(invokeMessage('expired', Date.now() - 1), channel);
    expect(execute).not.toHaveBeenCalled();
  });

  it('rejects attempts to suppress the local confirmation policy', async () => {
    const { execute, host, sent, channel } = fixture();
    await host.invoke(invokeMessage('no-confirmation', Date.now() + 10_000, false), channel);
    expect(execute).not.toHaveBeenCalled();
  ...
  it('rechecks foreground state locally', async () => {
    const { execute, host, channel } = fixture(false);
    await host.invoke(invokeMessage('background', Date.now() + 10_000), channel);
    expect(execute).not.toHaveBeenCalled();
  });

  it('does not execute when cancellation arrives during confirmation', async () => {
    const { execute, host, channel } = fixture();
    const invocation = host.invoke(invokeMessage('cancelled', Date.now() + 10_000), channel);
  ...
  it('does not report success when cancellation arrives during execution', async () => {
    ...
    const invocation = host.invoke(invokeMessage('cancelled-running', Date.now() + 10_000), channel);
    await Promise.resolve();
