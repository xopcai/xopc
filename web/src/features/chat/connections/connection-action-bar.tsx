...
const copy = {
  zh: { waiting: '等待连接', connect: '连接并继续', reconnect: '重新连接', details: '查看详情', skip: '跳过连接', cancel: '取消这个目标',
    ...
    replace: '更换应用', confirmReplace: '确认改用', apps: '个应用需要连接', preserved: '目标已保留', retry: '重试检查' },
  en: { waiting: 'Waiting for connection', connect: 'Connect and continue', reconnect: 'Reconnect', details: 'Details', skip: 'Skip connection', cancel: 'Cancel this objective',
    ...
    replace: 'Change app', confirmReplace: 'Confirm switch to', apps: 'apps need connecting', preserved: 'Objective preserved', retry: 'Retry check' },
};
...
export function ConnectionActionBar({ sessionKey }: { sessionKey: string }) {
  ...
  return <section className="mb-2 rounded-xl border border-edge bg-surface-panel px-3 py-2" aria-label={t.waiting}>
    <div className="flex flex-wrap items-center gap-2">
      ...
      <button type="button" className="min-w-0 flex-1 text-left text-sm text-fg" onClick={() => setOpen(true)}>
        {wait.phase === 'ready' ? t.ready : single ? `${t.waiting} · ${first.label}` : `${wait.needs.length} ${t.apps}`}
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[60] flex h-[min(90dvh,34rem)] w-[min(94vw,34rem)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-edge bg-surface-panel outline-none">
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
            <Dialog.Description className="text-sm text-fg-muted">{wait.phase === 'review_scope' ? t.scopeHint : t.intro}</Dialog.Description>
