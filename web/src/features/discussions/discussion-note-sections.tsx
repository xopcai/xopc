...
export function DiscussionNoteSections({ noteId }: { noteId: string }) {
  ...
  return <section className="relative flex h-full min-h-0 w-full flex-col text-fg">
    ...
    {editing ? <Dialog.Root open onOpenChange={open => { if (!open) setEditing(null); }}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[70] bg-surface-base/80" /><Dialog.Content aria-describedby={undefined} className="fixed left-1/2 top-1/2 z-[71] flex h-[min(28rem,90dvh)] w-[min(36rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-edge bg-surface-panel"><header className="flex items-center justify-between border-b border-edge p-4"><Dialog.Title>{zh ? '校对逐字稿' : 'Edit transcript'}</Dialog.Title><button aria-label={zh ? '关闭校对' : 'Close editor'} onClick={() => setEditing(null)}><X className="size-4" /></button></header><div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4"><label className="block text-xs">{zh ? '本段说话人' : 'Speaker for this segment'}<input className="mt-1 w-full rounded border border-edge bg-transparent p-2 text-sm" value={editing.speaker} onChange={event => setEditing({ ...editing, speaker: event.target.value })} /></label>{editing.segment.speakerLabel ? <label className="flex items-center gap-2 text-xs"><input type="checkbox" className="ui-checkbox" checked={Boolean(editing.applyToSpeakerGroup)} onChange={event => setEditing({ ...editing, applyToSpeakerGroup: event.target.checked })} />{zh ? '同时更名所有同标签片段；输入已有标签可合并说话人' : 'Rename all segments with this label; use an existing label to merge speakers'}</label> : null}<textarea className="min-h-32 w-full rounded border border-edge bg-transparent p-2 text-sm" value={editing.text} onChange={event => setEditing({ ...editing, text: event.target.value })} /></div><footer className="border-t border-edge p-3"><Button variant="primary" disabled={busy || !editing.text.trim()} onClick={() => void operation(async () => {
