  ...
  return (
    <Dialog.Root open={Boolean(comparison)} onOpenChange={(open) => { if (!open) onClose(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="xopc-dialog-content fixed left-1/2 top-1/2 z-50 flex h-[min(46rem,calc(100vh-2rem))] w-[min(64rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-edge bg-surface-overlay shadow-float">
