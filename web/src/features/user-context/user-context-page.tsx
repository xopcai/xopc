...
import {
  createCollaborationRule, deleteCollaborationRule,
  detectBrowserTimezone, fetchUserContext, fetchUserContextSettings,
  fetchConnectedContentCandidates, fetchUnderstandingSourceOverview,
  fetchUnderstandingSourceRevocationImpact,
...
const copy = {
  en: {
    ...
    profileHint: 'Facts you provide directly. When memory is enabled, voice calls can also use relevant details available when the call starts.',
  ...
  zh: {
    ...
    profileHint: '你直接提供的事实。开启记忆后，语音通话也会在接通时读取相关信息。',
...
function SourcesPanel({ t }: { t: typeof copy.en | typeof copy.zh }) {
  ...
  const [revokeImpact, setRevokeImpact] = useState<{ grantId: string; impact: SourceRevocationImpact } | null>(null);
  ...
  const revokeGrant = async (grant: UnderstandingSourceGrant, derived?: 'delete' | 'retain') => {
    if (confirmingRevoke !== grant.id) {
      try {
        const result = await fetchUnderstandingSourceRevocationImpact(grant.id);
        setConfirmingRevoke(grant.id);
        setRevokeImpact({ grantId: grant.id, impact: result.impact });
        setFeedback(null);
      } catch (impactError) {
        setFeedback({ tone: 'error', message: actionError(impactError) });
      }
      return;
    }
    if (!derived) return;
    ...
    try {
      await revokeUnderstandingSourceGrant(grant.id, { derived, raw: 'delete' });
      await mutate().catch(() => undefined);
      setConfirmingRevoke(null);
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-2"><h2 className="text-sm font-semibold text-fg">{t.authorizedSources} · {grants.length}</h2>
      {grants.length ? <div className="grid gap-3 sm:grid-cols-2">{grants.map((grant) => {
        ...
        return <article key={grant.id} className="flex min-h-28 flex-wrap items-start gap-3 rounded-2xl border border-edge bg-surface-panel p-4">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-1">{grant.accessMode === 'continuous' ? <Button variant="ghost" disabled={runningAction !== null} aria-busy={runningAction?.grantId === grant.id && runningAction.kind === 'refresh'} onClick={() => void refreshGrant(grant)}>{runningAction?.grantId === grant.id && runningAction.kind === 'refresh' ? <><Loader2 className="size-4 animate-spin" />{t.refreshing}</> : t.refresh}</Button> : null}<Button variant="ghost" className="text-danger" title={t.revokeHint} aria-label={`${t.revoke}: ${grant.displayName}`} disabled={runningAction !== null} aria-busy={runningAction?.grantId === grant.id && runningAction.kind === 'revoke'} onClick={() => { if (confirmingRevoke === grant.id) { setConfirmingRevoke(null); setRevokeImpact(null); } else void revokeGrant(grant); }}>{runningAction?.grantId === grant.id && runningAction.kind === 'revoke' ? <><Loader2 className="size-4 animate-spin" />{t.revoking}</> : confirmingRevoke === grant.id ? t.cancel : t.revoke}</Button></div>
          {revokeImpact?.grantId === grant.id ? <div className="basis-full rounded-xl border border-edge bg-surface-base p-3"><p className="text-xs leading-5 text-fg-muted">{t.revokeImpact.replace('{{count}}', String(revokeImpact.impact.derivedCount))}</p><div className="mt-2 flex flex-wrap justify-end gap-2"><Button variant="ghost" onClick={() => { setConfirmingRevoke(null); setRevokeImpact(null); }}>{t.cancel}</Button><Button variant="ghost" onClick={() => void revokeGrant(grant, 'retain')}>{t.retainDerived}</Button><Button variant="primary" onClick={() => void revokeGrant(grant, 'delete')}>{t.deleteDerived}</Button></div></div> : null}
        </article>;
