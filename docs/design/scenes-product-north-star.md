...
日期：2026-09-19。状态：长期产品方向与分阶段设计基线。

本文定义 xopc「场景」的长期用户价值、产品承诺、能力轮廓、北极星指标、核心领域边界和分阶段演进方式。它是后续场景产品、运行时、模板生态和相关交互设计的上位依据。
...
现有 Proactive、Heartbeat、Automation、Workflow、Skill、TaskRun、Inbox 和通知实现仍是迁移基线。本文描述的是目标形态，不表示所有能力已经实现；具体实现状态以交付记录和代码为准。

实施方案见 [场景系统技术设计与实施方案](./scenes-technical-design.md)。采用直接切换：不保留旧运行时、兼容 API、双写或回退适配；已有用户数据经一次性转换后进入新模型。下述研发阶段不代表线上新旧双轨运行。
...
| Workflow | 一组确定步骤怎样可靠执行？ | 场景可选择的执行器 |
...
| Heartbeat | 周期唤醒 Agent 检查清单 | 迁移为内置「日常巡查」场景 |
| Proactive | 当前事件、分析、卡片与策略基础 | 演进为场景运行时基础设施 |
...
| SceneWorkItem | 场景持续跟踪的一件现实事项，例如一次会议、一个邮件线程或一次家庭活动；不要求都是 Task |
| SceneEvent | 时间或业务变化形成的事实事件 |
| SceneRun | 一次触发后的执行、等待、审批和交付记录 |
| SceneOutcome | 结构化结果、证据、交付物和动作提议 |
| ScenePresentation | 结果在原场景、工作台、Inbox 和通知中的投影决定 |

第一阶段产品实现只需要明确 SceneTemplate、SceneActivation 和 SceneOutcome。SceneWorkItem、统一 SceneRun 和公共扩展协议按真实场景需要逐步抽取，不能为了模型完整度先行重写。
...
- 结果：短摘要、待决定事项或静默；
- 验证：现有 Heartbeat 一次性转换后删除旧运行时；空清单不消耗模型，静默与提醒符合策略。
...
阶段按用户闭环和验收条件推进，不按后台模块完成度推进。P0/P1/P2 的必要能力在首次正式切换前完成；技术研发顺序以技术方案 T0–T2 为准，先在隔离数据上形成新链路，再统一切换，不把旧执行器作为过渡上线方案。
...
- 设计现有项目、会议、邮件和 Heartbeat 到内置场景的一次性转换；
- 确定用户文案、场景承诺和“我的场景”视图；
- 建立 WSVR、首次价值、采用、打扰和安全事件埋点；
- 明确旧运行时、接口和配置删除清单，不增加兼容层。

完成条件：产品契约和数据转换方案可评审；用户无需理解 Proactive、Heartbeat 或订阅即可看懂场景承诺、范围、结果和暂停方式。
...
完成条件：四个场景走统一结果协议；重启、迟到事件、改期、来源陈旧、撤权和外部动作未知状态均有确定行为；旧运行时和入口删除，转换及各客户端验收通过后首次正式切换。若缩小首发模板范围，未支持实例须明确暂停，不以 legacy 执行器兜底。
...
2. **复用能力，不兼容旧链路**：复用 Agent、Workflow、TaskRun、Connector 和通知基础件；替换 Proactive、Heartbeat 专有运行时，不建立平行系统或适配层。
...
7. **没有首次价值就不扩模板数量**：先证明近期工作场景与生活试点，再做生态。
...
| `proactive_events` 与 temporal worker | 一次性转换事件记录，替换旧扫描器 | SceneEvent／TriggerIntent |
| proactive run／insight | 一次性转换历史，删除旧 reader | SceneRun／SceneOutcome |
| proactive card／Inbox | 统一结果卡和采用证据 | ScenePresentation |
| `HEARTBEAT.md` 和 HeartbeatService | 一次性导入私人说明，删除旧运行时与文件驱动 | 内置周期场景 |
...
迁移必须满足：一个触发实例只有一个所有者；不保留新旧双轨、双写或兼容接口；现有用户权限不扩大；旧成果与运行记录转换为新模型后可读；外部动作不能因迁移或回滚重复执行。历史 migration 和用户备份不是运行时兼容层，不能以清理 legacy 为由无条件删除用户数据。
