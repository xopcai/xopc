...
2. **Multi-project understanding:** selected projects receive separate bounded content analyses in ranking order; unselected ranked projects contribute metadata only. Ordinary user understanding is maintained automatically; eligible candidates can be used as labeled working assumptions and edited or deleted in You.
...
4. **Native personal context scan:** available only in the Electron app on macOS. The onboarding action requests the operating-system permissions for Notes, Mail, Calendar, and Reminders, then reads up to 50 recent items from each source with per-item and total character limits. Mail is limited to inbox and sent messages from the previous 30 days. The personal-context synthesis waits for the directory investigation and reconciles corroboration, conflicts, current focus, and stale signals across the sources. Raw source content is held only for the model call and is not stored in xopc's database.
...
Native app access uses fixed JXA programs through the public Automation interfaces for Notes, Mail, Calendar, and Reminders. No source values are interpolated into executable script text, and xopc does not read private application databases. Each scan is initiated by the onboarding action; there is no durable native-source grant or scheduled rescan. User-confirmed assertions remain user-controlled through User model.
...
flowchart TD
  ...
  Discover --> Native[macOS Notes, Mail, Calendar, and Reminders scan]
