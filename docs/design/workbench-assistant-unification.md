...
日期：2026-09-13。状态：已实现的过渡方案；产品方向由[场景优先方案](./proactive-scenario-product.md)继续修订。

过渡阶段完成了导航与深链统一、工作台注意力排序、事项详情、自然语言交代入口和产品化提醒文案。场景化重构进一步将工作台改为用户时刻，将持续事项列表降级为「助理安排」。后续扩展不恢复独立的主动性配置页或模板市场。
...
- **助理安排回答「助理长期记住了什么、被允许做到哪里」**，是需要时才进入的二级管理页。
...
「更多」中的**助理安排**用于调整长期安排；项目、会议、邮件和对话仍是建立安排的主要入口。
