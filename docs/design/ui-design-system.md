...
### 产品定位
xopc 工作站是为**一人公司（OPC）**打造的 AI 增强认知工作台。它不是一个工具，而是一个**个人操作系统** — 让一个人拥有团队的战斗力，通过知识管理、轻项目组织和 AI 催化，持续放大个人能力，形成基于自身数据的飞轮效应。
...
### 可选主题：暖瓷 · Warm Porcelain

外观设置中的 `porcelain` 为独立可选配色，不改变默认主题或已有晨曦主题。配色与浅色、深色、跟随系统模式分别保存。
...
| 次要文字 | `#665B54` | `#C5B5A8` |
| 主操作 | `#985D4D` | `#D4A08C` |
| 主操作悬停 / 按下 | `#874F41` / `#754337` | `#E2B39F` / `#C18D79` |
| 按钮文字 | `#FFFCF7` | `#211C19` |
| 链接 / 强调文字 | `#874F41` | `#E2B39F` |
| 选中背景 | `#E8D9CF` | `#493A32` |
| 边框 | `#E4DCD3` | `#4A3E35` |

- 应用壳使用燕麦 `#E9D8C8`、暖瓷 `#F6F0E7`、烟玫 `#E6D3D0` 的边缘柔光；正文面板和弹层保持实色，避免渐变干扰阅读。
- 本主题以陶棕替代默认品牌蓝，用于主操作、链接与选中提示；成功、警告、错误继续使用独立语义色。
- 深色采用暖褐底色与逐级提亮的表面，单独调整文字和操作色，不反转浅色渐变。
- 沿用系统字体、圆角与布局；颜色和阴影复用 `globals.css` 的现有语义 token。
