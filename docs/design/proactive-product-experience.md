...
本文重新确定主动性的产品目标、交互方式和交付顺序。[已有模板与卡片方案](./proactive-templates-and-cards.md)继续作为已交付工程能力的记录；后续产品设计以本文为建议方向，不把旧文档中的“可用版本”解释为已经达到用户产品的可用性。
...
保留 xopc 现有项目、任务和对话，把主动助理融入现有首页工作台。主导航中的“今天”承接工作台价值；现有聊天入口保持容易访问，不强制把所有会话合并成一条。`/proactive` 保留兼容直达链接，逐步转为事项详情与高级管理入口。
