...
| Section | Purpose |
| --- | --- |
| `agents` | Agent manifests, capability presets, default agent id |
...
There is no `agents.defaults` merge layer. Use `agents.capabilityPresets` for reusable policy patches and `agents.list[]` for concrete agents.
