...
[frpc-config.ts](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/tunnel/frpc-config.ts#L21) 生成的配置没有 `transport.tls.trustedCaFile`；[frps.toml](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/frps.toml#L3) 未配置服务端 TLS 证书/密钥及强制 TLS。
...
[下载地址与缓存](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/tunnel/frpc-binary.ts#L55) 按平台镜像、GitHub、`ghfast.top` 回退；[下载执行路径](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/tunnel/frpc-binary.ts#L142) 只判断缓存文件存在，下载成功后直接解压、chmod 并交给隧道进程执行。未见随版本固定的摘要或签名验证。
...
[registerTunnel](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/tunnel-service.ts#L70) 允许申请任意未占用的 4–12 位字母数字子域名；[过期清理](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/index.ts#L15) 的默认租约为 24 小时，周期删除过期记录。删除后没有绑定历史所有者的保留记录。停止客户端、长期离线或主动释放后，原域名可被别人申请。
...
[agent-avatar-display.tsx](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/web/src/features/settings/agents/agent-avatar-display.tsx#L48) 构造 `/api/agents/:id/avatar?token=...`。[auth.ts](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/gateway/hono/middleware/auth.ts#L58) 特别允许此路径使用 query token；同一 token 在其他接口具有 owner 权限，并非仅可读头像的凭据。

[Nginx wildcard 配置](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/nginx/wildcard-tunnel-proxy.conf#L20) 未显式配置脱敏日志。若生产继承常规 combined 日志，完整请求行连同 token 会落盘；[Nginx 官方日志定义](https://nginx.org/en/docs/http/ngx_http_log_module.html) 包含 `$request`。此时仅有日志读取权限的人也可能获得 Gateway 管理员凭据，不必控制反向代理进程。未读取生产日志，所以没有认定实际泄露事件已经发生。
...
[Console 注销](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/console/server/src/index.ts#L251) 只删除 API key；[key-revocation.ts](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/console/server/src/key-revocation.ts#L8) 不操作 Broker 隧道。[heartbeatTunnel](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/tunnel-service.ts#L167) 只验证 tunnelToken 和租约，并把到期时间滚动延长；[FRP Ping](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/tunnel-service.ts#L190) 也不检查注册密钥/用户/租户状态。
...
[Nginx](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/nginx/frp-tunnel-proxy-params.conf#L6) 用 `$proxy_add_x_forwarded_for` 保留并追加用户传入的 XFF；[Broker](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/register-rate-limit.ts#L59) 直接取第一项。攻击者换一个 XFF 即获得新的 IP 限流桶。本地将阈值设为 1，得到相同 XFF 的 200→429，换 XFF 后又是 200。

[Gateway auth](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/gateway/hono/middleware/auth.ts#L90) 在未设置 trustedProxies 时同样直接信任转发头；[loopback helper](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/gateway/security/loopback.ts#L41) 取第一项。默认允许 loopback 免限流，伪造 `127.0.0.1` 能持续得到 401，而普通 IP 已被 429 限制。认证本身仍会拒绝错误 token，不能将限流绕过描述为认证绕过。

此外，[stream 转发](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/nginx/stream-tls-route.conf#L19) 未携带 PROXY protocol，内层 Nginx 的 `$remote_addr` 在此模板链路上成为本机回环地址：即使客户端不伪造，也可能导致共用限流桶或被 Gateway 误认为 loopback。Broker 的 IP/hash Map 没有容量上限或过期删除，持续制造新 key 还会放大内存压力。
...
[启动守卫](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/tunnel/consent.ts#L63) 只检查风险同意；[手动 start API](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/gateway/hono/routes/tunnel.ts#L228) 只提取非空请求凭据，不检查实际 resolved auth mode。[CLI](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/cli/commands/tunnel.ts#L37) 检查 token 配置存在，但不检查 `auth.mode`。

本机 loopback 的 `auth.mode=none` 是允许配置；若仍残留 `gateway.auth.token`/环境 token 且已有注册密钥、同意记录，CLI 可进入 tunnel.start。HTTP 手动路径也可在 none 模式下携带任意非空 Bearer 进入启动处理。FRP 最终仍转发到该无鉴权 Gateway；[none 分支](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/gateway/hono/middleware/auth.ts#L222) 将请求赋予 owner/admin。隧道登记时 hash 了一个字符串，不代表目标 API 正在验证该字符串。
...
[frp-handler.ts](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/frp-handler.ts#L56) 与 [verifyFrpcNewProxy](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/tunnel-service.ts#L208) 只检查 token、proxyName、subdomain，忽略 `proxy_type`、`remote_port`、custom_domains、group 等。带合法身份字段的 `proxy_type=tcp` 请求在本地回调被放行。
...
[registerTunnel existing 分支](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/apps/broker/src/tunnel-service.ts#L62) 对同一个 gatewayTokenHash 直接删除旧记录并签发新 token；只在双方 workspaceId 非空且不同时拒绝。没有要求原 tunnelToken、原设备签名或 principal 一致。
...
平台 Nginx 能看到 HTTP 正文、Authorization、上传附件、返回内容和 WebSocket 数据。Broker 应用数据库并不因此自动存有所有聊天正文；需要区分“经过/可读取”和“已持久化”。平台主机/代理被控制后，攻击者可修改转发中的请求，正常 Bearer access token 没有请求签名约束。[手机默认 scopes](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/gateway/security/gateway-scopes.ts#L24) 允许会话读写、工作区读写、运行 Agent 与自动化，虽然没有 gateway.admin，泄露影响仍很大；能否进一步操作主机文件/工具还取决于 Agent 具体权限。

若产品的隐私目标是“云端看不到个人数据”，当前 broker_terminated 路径不满足。应选择到 Gateway 的 TLS 透传，或手机与 Gateway 间经过身份认证的端到端加密；平台只处理路由与必要元数据。单纯打开 FRP useEncryption 或加一次签名 probe，无法隔离作为合法 TLS 终止方的平台。历史 E2EE 分支在 [部署说明](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/docs/e2ee-branch-deploy.md#L26) 中被标为归档，不能当作当前保护。

其他项：

- [frps systemd 单元](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/systemd/frps.service#L5) 没有 `User=` 和进程沙箱配置；按模板部署为 root，网络进程漏洞的后果被放大。实际线上是否有 override 未核实。应使用专用低权限用户、限制文件与能力，并隔离证书、Broker/Auth 数据库、下载镜像写权限。
- [frps.toml](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/frps.toml#L3) 的绑定和 [Nginx 内层 listener](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/deploy/broker/nginx/wildcard-tunnel-proxy.conf#L11) 依赖外部防火墙隔离。仓库确有 [防火墙脚本](https://github.com/xopcai/xopc-platform/blob/ecb3fb34402d5e707611c292055bbef55b33d140/scripts/broker/apply-vps-firewall.sh#L17)，本次 8080/7100 无法直连，不作为已确认公网泄露报告；仍建议能绑 loopback 的内部端口直接绑定 loopback，并核实云安全组/IPv6。
- [本地 tunnel.json](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/tunnel/tunnel-state.ts#L26) 与 [frpc TOML](https://github.com/xopcai/xopc/blob/8323c54b3bfa33d5596d36c8ec318e1392e7ab24/src/tunnel/frpc-config.ts#L37) 没有显式 0600，旧 TOML 未随 stop/release 清理。本机只检查权限元数据：状态文件和多个 TOML 为 0644，但父目录 `~/.xopc` 为 0700，因此不能声称其他本机用户当前就能读取。建议显式 0600、原子写与旧凭据清理，覆盖自定义 state dir 和备份场景。
