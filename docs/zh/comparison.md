...
如果你想要的是一个自托管、本地优先、能持续维护 Task 与 Project 上下文，并且贯穿模型、通道、自动化、工作流和多端入口的个人 AI 系统，**xopc** 更适合作为底层。
...
| **Claude Code** | Anthropic 的 agentic coding system：读代码库、改文件、跑测试、处理 git 和工具链。 | 项目级工程任务和编码任务托付。 | xopc 是面向长期工作的个人 AI 运行时，不只是一轮 repo 执行。Task、Project、会话、Agent、通道、技能、工作流和自动化可以围绕同一套本地状态连接起来。 |
...
- 你需要 Task、Project、会话和周期性工作的长期上下文。
