...
在 **设置 → 能力 → 语音** 开启实时语音。进入已有聊天，在通话按钮旁选择：

- **语音助手**：转写 → 现有 Agent → 流式朗读，可以使用工具和审批流程。
- **自然聊天 · 无工具**：直接使用 Qwen 原生音频对话，不调用 Agent 或工具。

麦克风按钮仍然用于可编辑的实时转写，不自动发送消息。

自然聊天在同一设置页单独配置。选择 **XOPC Platform** 使用已有 XOPC 登录授权，或选择 **DashScope** 使用自己的 API Key / `DASHSCOPE_API_KEY`。当前支持 `qwen3-omni-flash-realtime`，不依赖 STT/TTS 配置。每次通话重新开始，不自动读取 Agent 历史。

平台模式需要管理员先在 **模型与路由 → 语音聊天** 选择已有 DashScope 连接，确保密钥健康，明确填写四类 Token 单价并发布。**语音调试台 → 自然聊天** 使用同一转发链路；用量缺失的通话会保留额度预占，等待账单核对。

切换模式前先结束通话。“静音”只关闭扬声器，不关闭麦克风。打断后的记录可能包含尚未播放的文字。协议已升级为 v2，网页和网关应同时更新。
