...
- 弹窗表单：Bot Token、白名单、启用开关及 **高级选项**（API 根地址、HTTP 代理、策略、多账号 JSON 等）。
- **保存** 通过 `PATCH /api/config` 写入 `channels.telegram`，并保留配置中其它字段。

### 命令行配置（与网关共用配置文件）

网关与 CLI 使用同一份 JSON（默认 `~/.xopc/xopc.json`；也可用环境变量 `XOPC_CONFIG` 或命令行全局参数 `--config` 指定）。可在不使用浏览器控制台的情况下配置通道：
...
通过 CLI 修改凭据或启用状态后，若网关已在运行，请 **重启或热加载网关**，以便通道进程加载新配置。
...
在需要 **@mention** 的 Telegram **群聊/超级群** 中，**仅语音**消息会先转写再做过滤，以便口播中的 @ 或类似说法能被识别；详见 [语音功能](/zh/voice)。
...
- ✅ 通过网关聊天（REST；助手回复在 `/api/agent` 上以 **SSE** 流式输出）
- ✅ 会话管理（`#/sessions`、侧栏对话列表）
- ✅ **即时通讯** 页 `#/channels`，配置 Telegram 与微信（见上文）
- ✅ 其它设置（模型、网关访问令牌、语音等）
- ✅ 日志查看
- ✅ Cron 任务管理
