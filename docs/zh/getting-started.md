...
xopc 是一个本地优先、逐渐理解你的私人 AI 助手。它只从你主动提供的上下文和你们共同完成的工作中开始认识你。第一次使用应保持简单：安装一个客户端、连接一个模型、先建立一次可用的对话，再决定是否开放更多能力。
...
连接成功后，不必立刻配置所有功能。先给 xopc 一小段真实且重要的上下文，例如：
...
启用用户理解后，可以在 **你** 或 **用户上下文** 中查看等待确认的理解。只确认准确且有用的内容；其它内容可以纠正、拒绝或删除。详见[用户理解](./user-understanding.md)。
