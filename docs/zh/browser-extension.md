...
- 文件、截图、PDF、Office 文档与代码附件；最多 10 个，每个不超过 32 MiB。截图和图片可在发送前预览，长文本粘贴会自动转为附件。
- 最多两个网页或选区上下文，支持 `@` 选择标签页、预览、移除和刷新。
- 中文输入法保护、按 Gateway/会话隔离的持久草稿、运行中追加队列、编辑/取消待发消息；Ctrl/⌘+Enter 停止当前回复后发送。
- 可搜索的模型和思考强度选择、`/` 命令/技能面板，以及语音转文字（需要 Gateway 已配置语音识别）。
...
第一次在侧栏截图时，Chrome 会请求所有网站访问权限。`captureVisibleTab` 要求 `<all_urls>` 或当前页已获临时 `activeTab` 授权，仅授权一个网站不足以支持持续使用侧栏截图。该权限是可选的；拒绝后仍可继续文字聊天。

语音输入会请求麦克风权限，最长录制两分钟。点击完成后转写结果进入当前草稿，不会自动发送；取消或切换会话会释放麦克风。转写失败时可重试。

关闭并重新打开侧栏后会恢复草稿；其中网页引用标记为待刷新，防止把旧页面当成当前页面发送。解绑 Gateway 会清除本地草稿和待投递消息。
...
安装并连接后，可以用一篇 [MDN 异步 JavaScript 文档](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Async_JS/Introducing) 试试。原文留在左侧，xopc 对话放在右侧，方便边读边核对。

> 以下实拍图使用中文界面，拍摄时 Gateway 离线，仅展示页面附加和提问草稿，没有发送问题或生成回复。实际提问前，请先确认侧栏已连接 Gateway。

### 附加当前页面

1. 打开要阅读的网页，点击 Chrome 工具栏里的 **xopc** 图标。
2. 点击侧栏输入框旁的 **＋ → 当前页面**；Chrome 询问时，允许访问当前站点。
3. 确认输入框上方出现页面标题和域名，再输入问题并发送。

例如：“我会基础 JavaScript，请结合这篇文章解释异步解决了什么问题，并告诉我应该先读哪一节。”

![MDN 文档旁的 xopc 侧栏：输入框上方已附加页面来源，下方是阅读问题草稿](../assets/browser-extension/page-context.jpg)

*附加后的页面来源会显示在输入框上方，发送前可检查是否选对文章。*

### 只问看不懂的段落

1. 在网页中选中一段文字。
2. 点击侧栏的 **＋ → 选中的文本**，确认来源标签显示为选中文本。
3. 输入具体问题，例如：“用餐厅点餐的例子解释这段话，并指出这个类比哪里不够准确。”

![左侧选中 MDN 文档中的异步解释，右侧 xopc 已附加选文并填入解释问题草稿](../assets/browser-extension/selection-context.jpg)

*附加选文时分享的是选中的内容；需要联系文章上下文时，再附加当前页面。*

读完后，可以在同一段对话里继续要求：“把刚才讨论的内容整理成 3 条笔记，附上原文链接。”笔记会输出在对话中，需要时复制保存。
