...
db.exec(readFileSync(new URL('../src/scenes/schema.sql', import.meta.url), 'utf8'));
db.exec(`CREATE TABLE connector_accounts(id TEXT, principal_id TEXT, current_connection_id TEXT, enabled INTEGER, connector_id TEXT, allowed_agent_ids_json TEXT);
  ...
  INSERT INTO connector_installations VALUES ('installation', 'gateway-owner', 'gmail', 1, '[]', 'null');
  INSERT INTO connector_accounts VALUES ('personal', 'gateway-owner', 'connection', 1, 'gmail', NULL);
  INSERT INTO connector_connections VALUES ('connection', 'personal', 'active', NULL, 'gateway-owner', 'gmail', 'installation');`);
...
try {
  ...
  assert.equal(repository.listActivations({ ownerId: 'gateway-owner', workspaceId: 'smoke-workspace' }).length, 1);
  ...
  const active = repository.listActivations({ ownerId: 'gateway-owner', workspaceId: 'smoke-workspace' })[0];
