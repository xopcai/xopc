...
describe('prepare-electron-pack-dir', () => {
  ...
  it('stages minimal runtime node_modules when app artifacts exist', () => {
    ...
    expect(existsSync(join(packDir, 'node_modules/tar'))).toBe(false);
    expect(existsSync(join(packDir, 'node_modules/@huggingface/transformers'))).toBe(false);
    expect(existsSync(join(packDir, 'node_modules/onnxruntime-node'))).toBe(false);
    expect(existsSync(join(packDir, 'node_modules/sherpa-onnx-node'))).toBe(false);
    expect(existsSync(join(packDir, 'node_modules/node-pty'))).toBe(true);
    expect(existsSync(join(packDir, 'node_modules/playwright-core'))).toBe(false);
    expect(existsSync(join(packDir, 'node_modules/@larksuiteoapi'))).toBe(false);
    expect(
      existsSync(
        join(packDir, 'node_modules/node-pty/prebuilds', `${process.platform}-${process.arch}`),
      ),
    ).toBe(true);
    for (const platformArch of ['darwin-arm64', 'darwin-x64', 'win32-arm64', 'win32-x64']) {
      if (platformArch !== `${process.platform}-${process.arch}`) {
        expect(existsSync(join(packDir, 'node_modules/node-pty/prebuilds', platformArch))).toBe(false);
      }
    }
