...
useGatewayStore.setState({conversationId:'synthetic-test'});useLocaleStore.setState({language:'zh'});useSettingsModeStore.setState({mode:'advanced'});
...
if(path==='/api/voice/realtime/sessions'){const req=r.request().postDataJSON();session={sessionId:randomUUID(),ticket:'synthetic-ticket-0000000000000000',ticketExpiresAt:new Date(Date.now()+60000).toISOString(),protocolVersion:3,connectionEpoch:7,purpose:req.purpose,...(req.purpose==='conversation'?{mode:req.mode??'assistant'}:{}),inputMode:'server_vad',bargeIn:true,inputFormat:{encoding:'pcm_s16le',sampleRate:16000,channels:1},media:{transport:'websocket-pcm',codec:'pcm_s16le',frameDurationMs:20},limits:{maxSessionMs:600000,maxBinaryFrameBytes:65536,idleTimeoutMs:60000},websocketPath:'/api/voice/realtime/v3/ws',route:req.purpose==='dictation'?{engine:'dictation',stt:route.stt}:route};payload=session;connections.push(req)}
...
if(path==='/api/voice/realtime/status')payload={enabled:true,defaultMode:'assistant',stt:route.stt,tts:route.tts,omni:{provider:'alibaba',model:'test',managed:false},capabilities:{dictation:{available:true},assistant:{available:true},natural:{available:true},languages:['zh','en'],bargeIn:true,mediaTransports:['websocket-pcm']}};
if(path.startsWith('/api/clarifications/'))answers.push(r.request().postDataJSON());
...
await page.getByText('请选择路线',{exact:true}).waitFor();if(answers.length)throw Error('Clarification submitted without a click');await page.getByRole('button',{name:'路线 A',exact:true}).click();await page.waitForFunction(()=>!document.body.textContent.includes('请选择路线'));if(answers.length!==1||answers[0].answer!=='路线 A')throw Error('Clarification did not use existing endpoint');current.emit('task.activity',{taskId:'task1',toolCallId:'tool1',toolName:'clarify',status:'completed'});current.emit('task.done',{taskId:'task1',status:'completed'});current.emit('response.done',{responseId:'r2',audio:false,finishReason:'text_only'});
